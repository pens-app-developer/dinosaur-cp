...
class MainActivity : AppCompatActivity() {
    ...
    private fun setTitleCharacter() {
        for (imageCharacterId in 0..maxStage) {
            // クリア済みのステージ分はタイトル画面にキャラクターを表示
            if (clearStageId >= imageCharacterId) {
                if (imageCharacterId > 0) {
                    val imageCharacter: ImageView = findViewById(
    ...
    private fun setStageSelectButton() {
        for (imageCharacterId in 0..maxStage) {
            // クリア済みの次のステージまでボタンを有効化する
            if (clearStageId >= imageCharacterId) {
                val buttonStageSelectId = imageCharacterId + 1
                if (buttonStageSelectId <= maxStage) {
                    val buttonStageSelect: Button = findViewById(
                        resources.getIdentifier(
                    ...
                    // ボタンのテキストをクリア済みの色に変更
                    buttonStageSelect.setTextColor(
                        ContextCompat.getColor(
                            this,
                            R.color.colorButtonClearText
                        )
                    )
                    // ボタンの有効化
    ...
    private fun onCollectionClick() {
        ...
        if (!onTapDisabled) {
            ...
            onTapDisabled = true
            // チュートリアルに移動
            val intent = Intent(application, CollectionActivity::class.java)
            startActivity(intent)
            // BGM削除
            releaseMediaPlayer()
            // 効果音の削除
